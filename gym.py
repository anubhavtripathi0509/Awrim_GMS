...
from cloudsheets import insertSheet, updateSheet, deleteSheet
import threading
...
def check_date():
    ...
    for registration in registrations:
        ...
        end_date=registration[17]
        contact_no=registration[10]
        to=registration[11]
        ...
        if datetime.strptime(end_date, '%Y-%m-%d') < current_date:
            ...
            subject="Awrim Gym Membership"
            sms_message="Your gym membership has expired. Renew your subscription to continue accessing Awrim Gym."
            send_sms_notification(contact_no, sms_message)
            if '@gmail.com' in to:
                send_email_alert(subject, sms_message, to)


class MainApp(ctk.CTk):
    threading.Thread(target = check_date).start()
    def __init__(self):
        super().__init__()

        # Fixed size of the window, and cannot be resized
        self.resizable(False, False)
        self.title("Awrim Gym Management System")
        ...
        self.frame_9_button=ctk.CTkButton(
            self.navigation_frame, corner_radius=0, height=50, border_spacing=10,
            text="Enquiry Section",
            fg_color="transparent", text_color=("gray10", "gray90"),
            hover_color=("gray70", "gray30"),
            image=self.add_user_image, anchor="w", command=self.frame_9_button_event)
        self.frame_9_button.grid(row=9, column=0, sticky="ew")

        self.frame_10_button=ctk.CTkButton(
            self.navigation_frame, corner_radius=0, height=50, border_spacing=10,
            text="SMS",
            fg_color="transparent", text_color=("gray10", "gray90"),
            hover_color=("gray70", "gray30"),
            image=self.sms_image, anchor="w", command=self.frame_10_button_event)
        ...
        self.logout_button=ctk.CTkButton(
            ...
            command=self.logout)
        self.logout_button.grid(row=12, column=0, padx=20, pady=5, sticky="ew")
        ...
        # 9
        self.nineth_frame=ctk.CTkFrame(self, corner_radius=0, fg_color="transparent")
        #10
        self.tenth_frame=ctk.CTkFrame(self, corner_radius=0, fg_color="transparent")
        ...
        create_enquiry_section_frame(self.nineth_frame)
        create_sms_section_frame(self.tenth_frame)
    ...
    def frame_10_button_event(self):
        self.select_frame_by_name("frame_10")
...
def create_home_frame(home):
    months, visitor_incomes, member_incomes = [], [], []
    ...
    member_panel_frame1=ctk.CTkFrame(panel_frame, fg_color="#00C957")
    member_panel_frame1.grid(row=1, column=0, padx=10, pady=10, sticky="nsew")
    members_label=ctk.CTkLabel(member_panel_frame1, text="Today's Attendance", font=("Arial bold", 14))
    members_label.pack(pady=5, padx=60, anchor="w")
    # create a counter label to display the no. of members
    members_counter_label1=ctk.CTkLabel(member_panel_frame1, text="", font=("Arial bold", 50))
    members_counter_label1.pack(pady=10, padx=10, anchor="center")
    # create a counter label to display the no. of gym equipment
    # attendance_counter_label=ctk.CTkLabel(member_panel_frame1, text="", font=("Arial bold", 50))
    # attendance_counter_label.pack(pady=10, padx=10, anchor="center")
    ...
    #     members_count=cursor.fetchone()[0]
    #     attendance_counter_label.configure(text=members_count)
    #     conn.close()
    #     home.after(1000, get_todays_attendance_count)
...
def fetch_trainers_from_db():
            ...
            # Fetch all the trainer names
            trainer_names = [row[1] for row in cursor.fetchall()]
...
class RegistrationFrame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        label=ctk.CTkLabel(self, text="Awrim Gym Membership Registration", font=("Arial bold", 26))
        ...
        middle_name_entry=ctk.CTkComboBox(personal_info_frame, values=['NA'])
        middle_name_entry.set('NA')
        ...
        address_entry=ctk.CTkComboBox(personal_info_frame, values=["NA"])
        address_entry.set("NA")
        address_entry.grid(row=8, column=1, padx=20, pady=5)

        # Medical Condition
        medical_condition_label=ctk.CTkLabel(personal_info_frame, text="Medical Condition:", font=label_font)
        medical_condition_label.grid(row=9, column=0, padx=20, pady=5, sticky="w")
        medical_condition_entry=ctk.CTkComboBox(personal_info_frame, values=["NA"])
        medical_condition_entry.set("NA")
        medical_condition_entry.grid(row=9, column=1, padx=20, pady=15)
        ...
        nationalities_list=["Select Nationality", "Indian", "American", "Other"]
        ...
        contact_no_entry=ctk.CTkEntry(contact_frame, placeholder_text="+91 9123456789")
        ...
        email_entry=ctk.CTkComboBox(contact_frame, values=["NA"])
        email_entry.set("NA")
        email_entry.pack(pady=0, padx=10, fill="x")

        # Emergency Contact No
        payment_label=ctk.CTkLabel(contact_frame, text="Payment:", font=label_font)
        payment_label.pack(pady=0, padx=10, anchor="w")
        payment_entry=ctk.CTkEntry(contact_frame, placeholder_text="Payment in Rs.")
        # payment_entry.set("NA")
        payment_entry.pack(pady=10, padx=10, fill="x")

        # Emergency Contact No
        payment_balance_label=ctk.CTkLabel(contact_frame, text="Balances:", font=label_font)
        payment_balance_label.pack(pady=0, padx=10, anchor="w")
        payment_balance_entry=ctk.CTkComboBox(contact_frame, values=["NA"])
        payment_balance_entry.set("NA")
        payment_balance_entry.pack(pady=10, padx=10, fill="x")
        ...
        subscription_plan_options=["1 Month", "3 Month", "6 Month", "Yearly"]
        ...
        self.subscription_plan_entry.set("1 Month")
        ...
        user_reference_entry=ctk.CTkComboBox(subscription_frame, values=["NA"])
        user_reference_entry.set("admin")
        user_reference_entry.grid(row=5, column=1, padx=20, pady=15)

        # Payment Mode
        payment_mode_label=ctk.CTkLabel(subscription_frame, text="Payment Mode:", font=label_font)
        payment_mode_label.grid(row=6, column=0, padx=20, pady=15, sticky="w")
        payment_mode_options=["Online", "Cash"]
        payment_mode_entry=ctk.CTkComboBox(subscription_frame, values=payment_mode_options)
        payment_mode_entry.set("Online")
        payment_mode_entry.grid(row=6, column=1, padx=20, pady=15)

        # personal trainer
        personal_trainer_label=ctk.CTkLabel(subscription_frame, text="Personal Trainer:", font=label_font)
        personal_trainer_label.grid(row=7, column=0, padx=20, pady=15, sticky="w")
        personal_trainer_options = fetch_trainers_from_db()
        personal_trainer_entry=ctk.CTkComboBox(subscription_frame, values=personal_trainer_options)
        personal_trainer_entry.grid(row=7, column=1, padx=20, pady=15)

        self.image_label = ctk.CTkLabel(subscription_frame, text="No Image")
        self.image_label.grid(row=8, column=0, padx=20, pady=15, sticky="w")
        # select_button = ctk.CTkButton(subscription_frame, text="Select Image")
        select_button = ctk.CTkButton(subscription_frame, text="Select Image", command=self.select_image)
        select_button.grid(row=8, column=1, padx=20, pady=15)
        capture_button = ctk.CTkButton(subscription_frame, text="Capture Image", command=self.capture_image)
        capture_button.grid(row=8, column=2, padx=20, pady=15)
        ...
        register_button=ctk.CTkButton(outer_frame, text="Register", fg_color="Green",
                                    ...
                                    #   command=threading.Thread(target = self.register_subscription).start())
        ...
        back_button=ctk.CTkButton(outer_frame, text="Back", fg_color="Red", text_color=("gray10", "gray90"),
        ...
        self.medical_condition_entry=medical_condition_entry
        ...
        self.payment_balance_entry=payment_balance_entry
        ...
        cursor.execute('''
                   CREATE TABLE IF NOT EXISTS registration (
                       ...
                       birth_date DATE,
                       address TEXT,
                       medical_condition TEXT DEFAULT 'NA',
                       nationality TEXT,
                       contact_no TEXT,
                       email TEXT,
                       payment TEXT,
                       payment_balance TEXT,
                       ...
                       payment_mode TEXT,
                       personal_trainer TEXT DEFAULT 'NA'
    ...
    #     image_path = f"./member_photo/captured_image.jpg"  # Change this path as needed
    ...
    #     # Display the captured image (you can modify this part based on your needs)
    ...
    def capture_image(self):
        ...
        if ret:  # Check if a frame was successfully captured
            image_path = f"./member_photo/captured_image.jpg"  # Define image path
            cv2.imwrite(image_path, frame)  # Save the image

            if image_path:
                self.display_image(image_path)  # Display the image (if needed)
        else:
    ...
    def display_image(self, file_path):
        ...
        self.image = original_image.resize((330, 330))
    ...
    def register_subscription(self):
        ...
        payment_balance=self.payment_balance_entry.get()
        subscription_id=self.set_subscription_id()
        subscription_plan=self.subscription_plan_entry.get()
        start_date=self.start_timestamp_entry.get()
        end_date=self.end_timestamp_entry.get()
        user_reference=self.user_reference_entry.get()
        payment_mode=self.payment_mode_entry.get()
        personal_trainer=self.personal_trainer_entry.get()
        image_data=self.image
            

        # Validate the data
        if not (first_name and last_name and age and sex and birth_date and address and medical_condition and
                nationality and contact_no and email and payment and payment_balance and
                subscription_plan and start_date and end_date and user_reference and payment_mode and personal_trainer and image_data):
        ...
        try:
            ...
            messagebox.showerror("Validation Error", "Contact No must be 10-digit number.")
        ...
        cursor.execute('''
            INSERT INTO registration (first_name, middle_name, last_name, age, sex, birth_date, address, medical_condition,
                                    nationality, contact_no, email, payment, payment_balance, subscription_id,
                                    subscription_plan, start_date, end_date, user_reference, status, payment_mode, personal_trainer)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?,?,?)
        ''', (first_name, middle_name, last_name, age, sex, birth_date, address, medical_condition, nationality, contact_no,
              email, payment, payment_balance, subscription_id, subscription_plan, start_date_str, end_date_str,
              user_reference, 'Ongoing', payment_mode, personal_trainer))
        ...
        cursor.execute(
            "SELECT * FROM registration WHERE subscription_id LIKE ?",
            ('%' + subscription_id + '%',))

        # Fetch the matching record
        record = cursor.fetchone()
        record_string=str(record)
        data_list = [item.strip(" '") for item in record_string[1:-1].split(",")]
        # print(data_list)
        insertSheet(data_list,'registration')
        conn.commit()
        conn.close()
        ...
        file_path=os.path.join(folder_path, f"awrim_{first_name}_{last_name}.png")
        qr_img.save(file_path)

        folder_path2="member_photo"
        os.makedirs(folder_path2, exist_ok=True)
        file_path2=os.path.join(folder_path2, f"awrim_{first_name}_{last_name}.png")
        self.image.save(file_path2)

        # After successful registration, send an SMS
        formatted_contact_no=self.contact_no_entry.get()
        # Assuming contact_no_entry contains the formatted phone number
        to=self.email_entry.get()
        subject="Awrim Gym Membership Registration"
        sms_message=f"Hello {first_name}!, You have Successfully Subscribed for {subscription_plan} Plan. Subscription ID:{subscription_id}. Start Date: {start_date} End Date: {end_date}, - Awrim Gym"
        ...
        for entry in [self.first_name_entry, self.middle_name_entry, self.last_name_entry, self.age_entry,
                      self.address_entry, self.medical_condition_entry, self.contact_no_entry, self.email_entry,
                      self.payment_entry, self.payment_balance_entry, self.subscription_id_entry,
...
class ViewFrame(ctk.CTkFrame):
    ...
    def __init__(self, master, **kwargs):
        ...
        # self.image_list = []
        # Define and configure widgets within the frame
        label=ctk.CTkLabel(self, text="Gym Member's Information", font=("Arial bold", 28))
        label.pack(pady=10, padx=10)

        # Create the search bar
        self.search_entry = ctk.CTkEntry(self, placeholder_text="Search", width=10)
        self.search_entry.pack(padx=5, pady=5, fill=tk.X, expand=True)
        # Create the search button
        search_button = ctk.CTkButton(self, text="Search", command=self.on_search)
        # search_button = ctk.CTkButton(self, text="Search")
        search_button.pack(padx=5, pady=5, side=tk.TOP)
        ...
        self.table=ttk.Treeview(table_frame, columns=(
            "ID", "First Name", "Last Name", "Medical Condition", "Contact No","Balance", "Subscription ID",
            "Start Date", "End Date", "Status", "Personal Trainer"), show="headings", height=10)
        ...
        self.table.heading("Medical Condition", text="Medical Condition")
        ...
        # self.table.heading("Payment Mode", text="Payment Mode")
        self.table.heading("Personal Trainer", text="Personal Trainer")
    ...
    def on_search(self):
        ...
        # Execute SQL query with a WHERE clause for searching
        cursor.execute(
            "SELECT id, first_name, last_name, medical_condition, contact_no, payment_balance, subscription_id, start_date, end_date, status, personal_trainer FROM registration WHERE first_name LIKE ? OR last_name LIKE ? OR status LIKE ?",
    ...
    # Function to bind to the Enter key event
    ...
    def download_invoice(self):
        ...
        if selected_item:
            ...
            if record_data:
                ...
                last_name=record_data[2]
                contact_no=record_data[4]
                payment=member_data[12]
                balance=record_data[5]
                subscription_id=record_data[6]
                subscription_plan=member_data[15]
                start_date=record_data[7]
...
def GenerateInvoice(self, id_value, first_name, last_name, contact_no, payment, balance, subscription_id, subscription_plan, start_date, end_date, personal_trainer, payment_mode):
    ...
    # company logo
    logo_path = "test_images/logo.png"
    logo_width = 2 * inch
    logo_height = inch
    ...
    pdf.drawString(1 * inch, 8 * inch, f"Subscription Id.: {subscription_id}")

    pdf.setFont("Helvetica", 10)  # Font for dates
    pdf.drawString(6 * inch, 8 * inch, f"Start Date: {start_date}")
    pdf.drawString(6 * inch, 7.8 * inch, f"End Date: {end_date}")
    ...
    pdf.drawString(1 * inch, 7 * inch, f"{contact_no}")

    if subscription_plan == "1 Month":
        membership_price = 500
    elif subscription_plan == "3 Month":
    ...
    if isinstance(personal_trainer, str):
            personal_trainer_amount = 0
    

    # - Table of items (description, price, quantity, total)
    items = [
        ["Description", "Month", "Price"],
        ["Membership", subscription_plan, membership_price],
        ["Personal Trainer", " ", personal_trainer_amount],
        ["Total", " ", f"Rs.{membership_price+personal_trainer_amount}"],
    ]
    ...
    table.wrapOn(pdf, 6 * inch, 5 * inch)
    ...
    pdf.drawString(6 * inch, 4.5 * inch, f"Rs.{payment}")

    # --- Total payment balance ---
    pdf.setFont("Helvetica-Bold", 12)
    pdf.drawString(1 * inch, 4.25 * inch, "Total Amount Balance:")
    pdf.setFont("Helvetica", 12)
    pdf.drawString(6 * inch, 4.25 * inch, f"Rs.{balance}")
    ...
    text_width = pdf.stringWidth(copyright_text, "Helvetica", 8)
    start_x = (pdf._pagesize[0] - text_width) / 2
...
class EditForm(ctk.CTkToplevel):
    def __init__(self, master, first_name, id_value, table_reference):
        ...
        labels=["First Name:", "Middle Name:", "Last Name:", "Age:", "Sex:", "Date of Birth:", "Address:", "Medical Condition:",
                "Nationality:", "Contact No:", "Email Address:", "Payment:", "Balance:", "Subscription ID:",
                "Subscription Plan:", "Start Date:", "End Date:", "User Reference:", "Status:", "Payment Mode:", "Personal Trainer:"]
        self.entry_fields=[]
        ...
        # Display the photo of the member inside the edit form
        photo_frame=ctk.CTkFrame(edit_frame)
        photo_frame.grid(row=21, column=1, rowspan=16, padx=10, pady=10)
        photo_label=ctk.CTkLabel(edit_frame, text="Photo:", font=("Arial bold", 16))
        photo_label.grid(row=21, column=0, padx=10, pady=10, sticky="w")

        # Display the photo from the member_photo folder based on the first name & last name of the member
        photograph_path = os.path.join("member_photo", f"awrim_{self.member_data[1]}_{self.member_data[3]}.png")

        # Check if the file exists
        if os.path.exists(photograph_path):
            photograph_image = Image.open(photograph_path)
        else:
            photograph_path = os.path.join("member_photo", "awrim_default.png")
            photograph_image = Image.open(photograph_path)
        photograph_image=photograph_image.resize((200, 200), Image.LANCZOS)
        photograph_image=ImageTk.PhotoImage(photograph_image)
        photograph_label=ctk.CTkLabel(photo_frame, text="", image=photograph_image)
        photograph_label.image=photograph_image
        photograph_label.pack(pady=10, padx=10)

        # Display the qr code of the member inside the edit form
        qr_code_frame=ctk.CTkFrame(edit_frame)
        qr_code_frame.grid(row=55, column=1, rowspan=16, padx=10, pady=10)
        label=ctk.CTkLabel(edit_frame, text="QR Code:", font=("Arial bold", 16))
        label.grid(row=55, column=0, padx=10, pady=10, sticky="w")

        download_button_frame=ctk.CTkFrame(edit_frame)
        download_button_frame.grid(row=71, column=1, rowspan=50, padx=10, pady=10)
        ...
        qr_code_path=os.path.join("member_qrcodes", f"awrim_{self.member_data[1]}_{self.member_data[3]}.png")
        # Check if the file exists
        if os.path.exists(qr_code_path):
            qr_code_image = Image.open(qr_code_path)
        else:
            qr_code_path = os.path.join("member_qrcodes", "awrim_default.png")
            qr_code_image = Image.open(qr_code_path)
        # qr_code_image=Image.open(qr_code_path)
    ...
    def download_qr_code(self):
        ...
        qr_code_path=os.path.join("member_qrcodes", f"awrim_{self.member_data[3]}.png")
        qr_code_image=Image.open(qr_code_path)

        # Assuming self.member_data[3] is the unique identifier for the member
        save_path=os.path.join(os.path.expanduser("~"), "Downloads", f"awrim_{self.member_data[3]}.png")
    ...
    def update_record(self):
        ...
        try:
            ...
            self.cursor.execute('''
                ...
                first_name=?, middle_name=?, last_name=?, age=?, sex=?, birth_date=?, address=?, medical_condition=?, nationality=?,
                contact_no=?, email=?, payment=?, payment_balance=?, subscription_id=?, subscription_plan=?, start_date=?,
                end_date=?, user_reference=?, status=?, payment_mode=?, personal_trainer=?
                WHERE id=?
            ''', (*updated_data, self.member_data[0]))
            merged_data = [str(self.member_data[0])] + updated_data[0:]

            # updateSheet(merged_data,str(self.member_data[0]),'id', 'registration')
            # Print the updated end_date to check if it's changed
            print(f"Updated end_date: {updated_data[16]}")
            original_filename=f"member_photo/awrim_{self.member_data[1]}_{self.member_data[3]}.png"
            new_filename=f"member_photo/awrim_{updated_data[0]}_{updated_data[2]}.png"
            try:
                os.rename(original_filename, new_filename)
                # print(f"File '{original_filename}' renamed to '{new_filename}' successfully.")
            ...
            end_date=datetime.strptime(updated_data[16], '%Y-%m-%d').date()
            current_date=datetime.strptime(updated_data[15], '%Y-%m-%d').date()
            ...
            if end_date <= current_date:
                ...
                formatted_contact_no=self.entry_fields[9].get()
                to=self.entry_fields[10].get()
                sms_message="Your gym membership has expired. Renew your subscription to continue accessing Awrim Gym."
                self.send_sms(formatted_contact_no, sms_message)
                if '@gmail.com' in to:
                    subject="Awrim Gym Membership Renewal"
                    self.email_alert(subject, sms_message, to)
            ...
            my_thread = threading.Thread(target=updateSheet, args=(merged_data,str(self.member_data[0]),'id', 'registration'))
            my_thread.start()
                ...
                if record_data:
                    ...
                    try:
                        ...
                        if os.path.exists(image_path1):
                            ...
                            print(f"Image deleted: {image_path1}")
                        else:
                    ...
                    try:
                        ...
                        conn.commit()  # Commit the changes to the database
                        # threading.Thread(target=deleteSheet, args=(str(id_value), 'id', 'registration',).start())
                        my_thread = threading.Thread(target=deleteSheet, args=(str(id_value), 'id', 'registration'))
                        my_thread.start()
...
class RenewSubscriptionFrame(ctk.CTkToplevel):
    def __init__(self, master, id_value, table_reference):
        ...
        # Create labels and entry fields for editing the record
        labels=["ID:", "Contact No:","Email:", "Subscription ID:", "Subscription Plan:", "Start Date:", "End Date:", "Payment Mode:", "Personal Trainer:"]
        self.entry_fields=[]
        ...
        # Fetch data for the specified member using the provided 'id_value'
        self.cursor.execute(
            "SELECT id, contact_no, email, subscription_id, subscription_plan, start_date, end_date, payment_mode, personal_trainer FROM registration WHERE id=?",
            (id_value,))
        self.member_data=self.cursor.fetchone()
    ...
    def renew_membership(self):
        ...
        try:
            ...
            if end_date <= datetime.now().strftime('%Y-%m-%d'):
                ...
                if '@gmail.com' in self.member_data[2]:
                    self.send_email_expiration(self.member_data[2], start_date, end_date)
            ...
            if '@gmail.com' in self.member_data[2]:
                subject="Awrim Gym Membership Renewal"
                self.email_alert(subject, sms_message, self.member_data[2])
            ...
            self.destroy()
    def send_email_expiration(self, to, start_date, end_date):
        # Send an SMS to the member for subscription expiration
        subject="Awrim Gym Subscription Expiration"
        body=f"Hello {self.member_data[2]}!, Your subscription is expiring soon. Expiration date: {end_date}."
        self.email_alert(subject, body, to)
...
class ScanFrame(ctk.CTkFrame):
        ...
        try:
            ...
            with sqlite3.connect('attendance_records.db') as conn_attendance:
                ...
                if attendance_type == "Time In":
                    ...
                    send_sms(contact_no,
                             f"Hello {first_name}!, You have Successfully Time In. Subscription ID:{subscription_id}. Time In: {current_datetime}, - Awrim Gym")
                    ...
                    send_sms(contact_no,
                             f"Hello {first_name}!, Thank you for coming. See you again! Subscription ID:{subscription_id}. Time Out: {current_datetime}, - Awrim Gym")
    ...
    def scan_qr_code():
        ...
        while True:
            ...
            # Check for the close button being pressed (this is platform-specific)
...
class RecordsFrame(ctk.CTkFrame):
    ...
    def create_ui_elements(self):
        ...
        # h_scrollbar = ttk.Scrollbar(table_frame, orient=tk.HORIZONTAL, command=self.records_table.xview)
        # h_scrollbar.pack(side=tk.BOTTOM, fill=tk.X)
        # self.records_table.configure(xscrollcommand=h_scrollbar.set)
...
class TrainerFrame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        emergency_contact_entry=ctk.CTkEntry(contact_frame, placeholder_text="+91 9123456789")
    ...
    def register_subscription(self):
        ...
        # Specify the file path to save the QR code in the folder
        file_path=os.path.join(folder_path, f"awrim_trainer_{last_name}.png")
        qr_img.save(file_path)

        # After successful registration, send an SMS
        formatted_contact_no=self.contact_no_entry.get()
        to = self.email_entry.get()  # Assuming contact_no_entry contains the formatted phone number
        subject = 'Awrim Gym Registration'
        sms_message=f"Hello {first_name}!, You are registered as a Trainer in Awrim Gym."
        self.send_sms(formatted_contact_no, sms_message)
        if '@gmail.com' in to:
            self.email_alert(subject, sms_message, to)
...
class EditTrainerForm(ctk.CTkToplevel):
    def __init__(self, master, first_name, id_value, table_reference):
        ...
        # Display the qr code from the member_qrcodes folder based on the last name of the member
        qr_code_path=os.path.join("trainer_qrcodes", f"awrim_trainer_{self.trainer_data[3]}.png")
        qr_code_image=Image.open(qr_code_path)
        qr_code_image=qr_code_image.resize((200, 200), Image.LANCZOS)
        qr_code_image=ImageTk.PhotoImage(qr_code_image)
        qr_code_label=ctk.CTkLabel(qr_code_frame, text="", image=qr_code_image)
    ...
    def download_qr_code(self):
        ...
        # Assuming self.member_data[3] is the unique identifier for the member
        save_path=os.path.join(os.path.expanduser("~"), "Downloads", f"awrim_trainer_{self.trainer_data[3]}.png")
        qr_code_image.save(save_path)
            ...
            if confirm:
                ...
                if record_data:
                    ...
                    try:
                        ...
                        print(delete_photo_records)
                        # image_path = f"member_photo/awrim_{delete_photo_records[0][0]}_{delete_photo_records[0][1]}.png"
                        image_path1 = f"trainer_qrcodes/awrim_trainer_{delete_photo_records[0][1]}.png"
                        if os.path.exists(image_path1):
                            # Delete the file
                            os.remove(image_path1)
                            print(f"Image deleted: {image_path1}")
                ...
                if status and status[0] in ['Active', 'On Leave']:
                    ...
                    with sqlite3.connect('trainer_attendance_records.db') as attendance_conn:
                        ...
                        if attendance_type == "Time In":
                            ...
                            send_sms(contact_no,
                                     f"Hello {first_name}!, You have Successfully Time In. Time In: {current_datetime}, - Awrim Gym")
                            ...
                            send_sms(contact_no,
                                     f"Hello {first_name}!, You have Successfully Time Out. Time In: {current_datetime}, - Awrim Gym")
...
class LogbookFrame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        contact_no_entry=ctk.CTkEntry(personal_info_frame, placeholder_text="+91 9123456789")
    ...
    def attend_log(self):
        ...
        try:
            ...
            self.send_sms(contact_no,
                          f"Hello {first_name}!, Thank You for attending Awrim Gym. Time In: {current_time}")
...
class RegisterEmployeeFrame(ctk.CTkFrame):
    ...
    def register_employees(self):
        ...
        # Specify the file path to save the QR code in the folder
        file_path=os.path.join(folder_path, f"awrim_employee_{last_name}.png")
        qr_img.save(file_path)

        # After successful registration, send an SMS
        formatted_contact_no=self.contact_no_entry.get()  # Assuming contact_no_entry contains the formatted phone number
        sms_message=f"Hello {first_name}!, You are registered as an Employee of Awrim Gym."
...
class EditEmployeeForm(ctk.CTkToplevel):
    def __init__(self, master, first_name, id_value, table_reference):
        ...
        # Display the qr code from the member_qrcodes folder based on the last name of the member
        qr_code_path=os.path.join("employee_qrcodes", f"awrim_employee_{self.employee_data[3]}.png")
        qr_code_image=Image.open(qr_code_path)
        qr_code_image=qr_code_image.resize((200, 200), Image.LANCZOS)
        qr_code_image=ImageTk.PhotoImage(qr_code_image)
        qr_code_label=ctk.CTkLabel(qr_code_frame, text="", image=qr_code_image)
    ...
    def download_qr_code(self):
        ...
        # Assuming self.member_data[3] is the unique identifier for the member
        save_path=os.path.join(os.path.expanduser("~"), "Downloads", f"awrim_employee_{self.employee_data[3]}.png")
        qr_code_image.save(save_path)
            ...
            if confirm:
                ...
                if record_data:
                    ...
                    try:
                        ...
                        delete_photo_records=cursor.fetchall()
                        print(delete_photo_records)
                        image_path = f"employee_qrcodes/awrim_employee_{delete_photo_records[0][1]}.png"
                        if os.path.exists(image_path):
                            # Delete the file
                            os.remove(image_path)
                            print(f"Image deleted: {image_path}")
...
# -------------------- FRAME 9 --------------------#
def create_enquiry_section_frame(frame_9):
    # Define the desired button width and height
    button_width=250
    button_height=250

    # Define the path to the directory containing your image files
    frame_9_icons=os.path.join(os.path.dirname(os.path.realpath(__file__)), "frame_7_icons")

    # Load and resize the images
    register_image=Image.open(os.path.join(frame_9_icons, 'register_black.png'))
    register_image=register_image.resize((button_width, button_height), Image.LANCZOS)

    view_image=Image.open(os.path.join(frame_9_icons, 'list_black.png'))
    view_image=view_image.resize((button_width, button_height), Image.LANCZOS)

    attendance_image=Image.open(os.path.join(frame_9_icons, 'scan_black.png'))
    attendance_image=attendance_image.resize((button_width, button_height), Image.LANCZOS)

    def register_enquiry():
        # When the "Register Members" button is clicked, create and show the registration frame
        registration_frame=RegisterEnquiryFrame(frame_9)
        registration_frame.pack(fill='both', expand=True)

    def view_enquiry_information():
        # When the "View Members" button is clicked, create and show the view members frame
        view_employee_frame=ViewEnquiryFrame(frame_9)
        view_employee_frame.pack(fill='both', expand=True)

    def take_enquiry_attendance():
        # When the "View Members" button is clicked, create and show the view members frame
        attendance_frame=EnquiryAttendanceFrame(frame_9)
        attendance_frame.pack(fill='both', expand=True)

    # Create the buttons with the resized images
    register_employee_button=ctk.CTkButton(
        master=frame_9,
        text="Register Enquiry",
        image=ImageTk.PhotoImage(register_image),
        compound=tk.TOP,
        command=register_enquiry,  # Call the function to open the frame
        width=button_width,
        height=button_height,
        fg_color="#00C957",
        text_color=("gray10", "gray90")
    ...
    view_employee_button=ctk.CTkButton(
        master=frame_9,
        text="View Enquiry Information",
        image=ImageTk.PhotoImage(view_image),
        compound=tk.TOP,
        command=view_enquiry_information,
        width=button_width,
        height=button_height,
        fg_color="#00C957",
        text_color=("gray10", "gray90")
    ...
    # attendance_employee_button=ctk.CTkButton(
    #     master=frame_9,
    #     text="Enquiry Attendance",
    #     image=ImageTk.PhotoImage(attendance_image),
    #     compound=tk.TOP,
    #     command=take_enquiry_attendance,
    #     width=button_width,
    #     height=button_height,
    #     fg_color="#00C957",
    #     text_color=("gray10", "gray90")
...
class RegisterEnquiryFrame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        super().__init__(master, **kwargs)

        # STEP 1: PERSONAL INFORMATION
        # Define and configure widgets within the frame
        label=ctk.CTkLabel(self, text="Register Enquiry", font=("Arial bold", 26))
        label.pack(pady=20, padx=10)
        ...
        # # Description
        # description_label=ctk.CTkLabel(personal_info_frame, text="Description:", font=label_font)
        # description_label.grid(row=9, column=0, padx=20, pady=5, sticky="w")
        # description_entry=ctk.CTkEntry(personal_info_frame, placeholder_text="Enter the description")
        # description_entry.pack(pady=10, padx=10, fill="x")
        ...
        # Emergency Contact No
        description_label=ctk.CTkLabel(contact_frame, text="Description:", font=label_font)
        description_label.pack(pady=0, padx=10, anchor="w")
        description_entry=ctk.CTkComboBox(contact_frame, values=["NA"])
        description_entry.set("NA")
        description_entry.pack(pady=10, padx=10, fill="x")
    ...
    def register_enquiry(self):
        ...
        # Validate the data
        if not (first_name and last_name and age and sex and birth_date and address and
                nationality and contact_no and email and description):
            messagebox.showerror("Validation Error", "All fields are required.")
            return
        ...
        cursor.execute('''
                    INSERT INTO enquiry (
                        first_name, middle_name, last_name, age, sex, birth_date, address,
                        nationality, contact_no, email, description, status
                    )
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
            first_name, middle_name, last_name, age, sex, birth_date, address,
        ...
        # Specify the file path to save the QR code in the folder
        file_path=os.path.join(folder_path, f"enquiry_{last_name}.png")
        qr_img.save(file_path)

        # After successful registration, send an SMS
        formatted_contact_no=self.contact_no_entry.get()  # Assuming contact_no_entry contains the formatted phone number
        subject="Enquiry Registration"
        sms_message=f"Hello {first_name}!, You are registered as an Enquiry Member of Awrim Gym."
        to=self.email_entry.get()
        self.send_sms(formatted_contact_no, sms_message)
        if '@gmail.com' in to:
            self.email_alert(subject, sms_message, to)
        ...
        # Show a success message
        messagebox.showinfo("Registration Successful", "Enquiry registered successfully!")
...
class ViewEnquiryFrame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        super().__init__(master, **kwargs)

        # Define and configure widgets within the frame
        label=ctk.CTkLabel(self, text="Enquiry Information", font=("Arial bold", 28))
        label.pack(pady=20, padx=10)
        ...
        # Get only the specific columns from the database
        cursor.execute(
            "SELECT id, first_name, middle_name, last_name, age, sex, contact_no, status, description FROM enquiry")
        records=cursor.fetchall()
        ...
        # Create a table to display the records
        self.table=ttk.Treeview(table_frame, columns=(
            "ID", "First Name", "Middle Name", "Last Name", "Age", "Sex", "Contact No", "Status", "Description"), show="headings", height=10)
        self.table.pack(side=tk.LEFT)
    ...
    def edit_record(self):
        ...
        if selected_item:
            ...
            if record_data:
                ...
                id_value=record_data[0]
                first_name=record_data[1]
                edit_enquiry_form=EditEnquiryForm(self, first_name, id_value, self.table)


class EditEnquiryForm(ctk.CTkToplevel):
    def __init__(self, master, first_name, id_value, table_reference):
        super().__init__(master)

        # Set the title for the edit form
        self.resizable(False, False)
        self.title("Edit Enquiry Record")
        self.geometry("500x550")
        ...
        # Fetch data for the specified member using the provided 'id_value'
        self.cursor.execute("SELECT * FROM enquiry WHERE id=?", (id_value,))
        self.enquiry_data=self.cursor.fetchone()

        if self.enquiry_data is None:
            messagebox.showerror("Enquiry Not Found", "Enquiry not found in the database.")
            self.destroy()
            return

        # Create and configure widgets within the edit form
        label=ctk.CTkLabel(self, text="Edit Enquiry Record", font=("Arial bold", 20))
        label.pack(pady=10)
        ...
        # Create labels and entry fields for editing the record
        labels=["First Name:", "Middle Name:", "Last Name:", "Age:", "Sex:", "Date of Birth:", "Address:",
                "Nationality:", "Contact No:", "Email Address:", "Description:", "Status:"]
        self.entry_fields=[]
        self.status_combobox=None  # Initialize status_combobox attribute
        ...
        for i, label_text in enumerate(labels):
            ...
            if label_text == "Status:":
                # Create a combo box for status
                status_values=["Reverted Back", "Not Reverted", "Not Interested"]
                self.status_combobox=ctk.CTkComboBox(edit_frame, values=status_values)
                self.status_combobox.grid(row=i, column=1, padx=10, pady=5, ipadx=10, ipady=3)
                self.status_combobox.set(self.enquiry_data[12])

                self.entry_fields.append(self.status_combobox)
            else:
                entry=ctk.CTkEntry(edit_frame)
                entry.grid(row=i, column=1, padx=10, pady=5, ipadx=10, ipady=3)
                entry.insert(0, self.enquiry_data[i + 1])
                self.entry_fields.append(entry)
        ...
        # Display the qr code from the member_qrcodes folder based on the last name of the member
        qr_code_path=os.path.join("enquiry_qrcodes", f"enquiry_{self.enquiry_data[3]}.png")
        qr_code_image=Image.open(qr_code_path)
        qr_code_image=qr_code_image.resize((200, 200), Image.LANCZOS)
        qr_code_image=ImageTk.PhotoImage(qr_code_image)
        qr_code_label=ctk.CTkLabel(qr_code_frame, text="", image=qr_code_image)
    ...
    def download_qr_code(self):
        ...
        # Assuming self.member_data[3] is the unique identifier for the member
        save_path=os.path.join(os.path.expanduser("~"), "Downloads", f"enquiry_{self.enquiry_data[3]}.png")
        qr_code_image.save(save_path)
        ...
        try:
            with sqlite3.connect('register_enquiry.db') as conn:
                ...
                cursor.execute('''
                    UPDATE enquiry SET 
                    first_name=?, middle_name=?, last_name=?, age=?, sex=?, birth_date=?, address=?, nationality=?,
                    contact_no=?, email=?, description=?, status=?
                    WHERE id=?
                ''', tuple(updated_data + [self.enquiry_data[0]]))
            ...
            if confirm:
                ...
                if record_data:
                    ...
                    try:
                        ...
                        print(delete_photo_records)
                        # image_path = f"member_photo/awrim_{delete_photo_records[0][0]}_{delete_photo_records[0][1]}.png"
                        image_path1 = f"enquiry_qrcodes/enquiry_{delete_photo_records[0][1]}.png"
                        if os.path.exists(image_path1):
                            # Delete the file
                            os.remove(image_path1)
                            print(f"Image deleted: {image_path1}")
                        ...
                        print(f"Error deleting image: {e}")
                    try:
                        cursor.execute("DELETE FROM enquiry WHERE id=?", (id_value,))
                        conn.commit()  # Commit the changes to the database
                        print("Record deleted successfully.")
                    except sqlite3.Error as e:
                        messagebox.showerror("Error", f"Error deleting record: {e}")
...
class EnquiryAttendanceFrame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        super().__init__(master, **kwargs)
        self.create_ui_elements()
    ...
    def take_attendance(self):
        # When the "Register Members" button is clicked, create and show the registration frame
        attendance_frame=EnquiryScanQrFrame(self)
        attendance_frame.pack(fill='both', expand=True)

    def view_attendance_records(self):
        # When the "Register Members" button is clicked, create and show the registration frame
        records_frame=EnquiryRecordsAttendanceFrame(self)
        records_frame.pack(fill='both', expand=True)
...
class EnquiryScanQrFrame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        super().__init__(master, **kwargs)
    ...
    def record_attendance(member_data, attendance_type):
        try:
            ...
            with sqlite3.connect('register_enquiry.db') as conn:
                ...
                # Check the status before recording attendance
                cursor.execute('SELECT status FROM enquiry WHERE first_name=? AND last_name=? AND contact_no=?',
                               (first_name, last_name, contact_no))
                status=cursor.fetchone()
                ...
                if status and status[0] in ['Reverted', 'Not Reverted']:
                    ...
                    with sqlite3.connect('enquiry_attendance_records.db') as attendance_conn:
                        ...
                        if attendance_type == "Time In":
                            attendance_cursor.execute('''
                                   INSERT INTO enquiry_attendance (first_name, middle_name, last_name, contact_no, time_in)
                                   VALUES (?, ?, ?, ?, ?)
                               ''', (first_name, middle_name, last_name, contact_no, current_datetime))
                            send_sms(contact_no,
                                     f"Hello {first_name}!, You have Successfully Time In. Time In: {current_datetime}, - Awrim Gym")
                ...
                elif status and status[0] == 'Not Interested':
                    messagebox.showwarning("Attendance Denied",
                                           f"Enquiry {first_name} {last_name}'s status is inactive. Attendance denied.")
...
class EnquiryRecordsAttendanceFrame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        super().__init__(master, **kwargs)
        ...
        label=ctk.CTkLabel(table_frame, text="Enquiry Attendance Records", font=("Arial bold", 28))
        label.pack(pady=20, padx=10)
    ...
    def load_attendance_records(self):
        ...
        try:
            cursor.execute(
                'SELECT first_name, middle_name, last_name, contact_no, time_in, time_out FROM enquiry_attendance')
            records=cursor.fetchall()
            ...
            messagebox.showerror("Error", f"Error fetching enquiry attendance records: {e}")
...
########################################################### SMS Frame ########################################################
def create_sms_section_frame(frame_8):
    # Create the main frame for account management
    form_frame=ctk.CTkFrame(frame_8)
    form_frame.pack(pady=10, padx=10, fill="both", expand=True)
    ...
    # Add Full Name entry
    subject_label=ctk.CTkLabel(sub_frame, text="Subject:", font=label_font)
    subject_label.grid(row=1, column=0, padx=20, pady=20)
    subject_entry=ctk.CTkEntry(sub_frame, placeholder_text="Enter the subject")
    subject_entry.grid(row=1, column=1, padx=20, pady=20, sticky="ew")

    sms_message_label = ctk.CTkLabel(sub_frame, text="Message:", font=label_font)
    sms_message_label.grid(row=2, column=0, padx=20, pady=20)
    # Use ScrolledText for a multiline text box
    sms_message_entry = scrolledtext.ScrolledText(sub_frame, wrap=tk.WORD, width=40, height=5)
    sms_message_entry.grid(row=2, column=1, padx=20, pady=20, sticky="nsew")  # Use sticky to expand with the window

    # Adjust row and column weights to make the text box resizable
    ...
    send_button=ctk.CTkButton(
        info_frame,
        text="Send",
        command=lambda: send_total_sms(
            subject_entry.get(),
            sms_message_entry.get("1.0", tk.END)
        )
    )
...
def send_total_sms(subject, body):
    ...
    for email in registered_user:
        if '@gmail.com' in email:
            msg['to'] = email
    server.send_message(msg)
    server.quit()
    messagebox.showinfo('Email sent successfull','Your message has been sent successfully.')
...
def forgot_password():
    ...
    while True:
        user_phone_number=simpledialog.askstring('Enter Phone Number', 'Enter your phone number (e.g., +9123456789):')
...
def create_login_window():
    def login():
        ...
        if user_data:
            ...
            def update_username_password(username, new_username, new_password):
                ...
                conn.commit()
                conn.close()
            if current_date == datetime(2024, 1, 26).date():
                update_username_password(username, new_username, new_password)
    ...
    login_window.title("Awrim Gym Management System")
    ...
    original_image = Image.open("test_images/gym_dark.png")

# Resize the image to 413x165
    resized_image = original_image.resize((413, 165), Image.ANTIALIAS)  # Use Image.ANTIALIAS for high-quality resizing
